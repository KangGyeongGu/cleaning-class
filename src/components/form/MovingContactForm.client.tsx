...
export function MovingContactForm(): React.ReactElement {
  ...
  }, [name, phone, serviceType, departure, destination, message]);
  ...
  useEffect(() => {
    if (isFailure && !hasTrackedError.current) {
      ...
      track({
        ...
        event_payload: { inquiry_type: "moving", error_kind: errorKind },
        path: currentPath(),
      });
    }
  ...
  return (
    <form action={formAction} className="space-y-6">
      <input type="hidden" name="inquiryType" value="moving" />
      ...
      <div className="space-y-5">
        ...
        <CustomDropdown
          label="서비스 종류"
          name="serviceType"
          options={MOVING_INQUIRY_OPTIONS}
